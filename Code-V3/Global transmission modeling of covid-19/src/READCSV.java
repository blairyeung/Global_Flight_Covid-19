...
public class READCSV {
    ...
    public static void main(String[] args) throws Exception{
        ...
        for (int i = 0; i < countrycode.size(); i++) {
            String path1 = "E:/Global Model/"+outdate[datenum]+"/";
            String path2 = ".csv";
            String code = countrycode.get(i);
            String path =path1 + code + path2;
            String outpath;
            if(code.equals("CA")||code.equals("US")||code.equals("BE")||code.equals("DE")||code.equals("FR")||code.equals("GB")||code.equals("IT")||code.equals("ES")){
                System.out.println(true);
                outpath ="E:/Global Model/Graph/"+outdate[datenum]+ " "+code + "TR" + path2;
            }
            else {
        ...
        for (int i = 0; i < countrycode.size(); i++) {
            ...
            for (int i1 = 0; i1 < observation; i1++) {
                ...
                if(i1==0){
                    ...
                    printers[i].print(dates.get(0+i1+15*datenum)+",");
                }
                ...
                avg[i1] = lineaverage;
                avg[i1] = median(Mean);
                //System.out.println("MEDIAN"+avg[i1]);
                ...
                if(i1!=0&&i1!=observation-1){
                    ...
                    printers[i].println();
                    //printers[i].println(c+"=RSQ(AY112:AY241,AZ112:AZ241)"+c);
                }
                if(i1==0){
                    System.out.println();
                    printers[i].println();
                    ...
                    int dev = (int) Math.abs(bufferd[observation-(136-15*datenum)-1]-bufferd2[observation-(136-15*datenum)-1]);
                    double Percentagedev = (double)dev/bufferd[bufferd.length-1];
                    if(bufferd2[bufferd2.length-1]==0||bufferd[bufferd.length-1]==0||bufferd[bufferd.length-1]<100||bufferd2[bufferd2.length-1]<100){
                        //pearson = 0;
                        dev = 0;
                        Percentagedev = 0;
                    }
                    ...
                    System.out.println("PEARSON");
                    Dev.add(dev);
                    PercentageDev.add(Percentagedev);
        ...
        System.out.println(Dev);
        File pearson = new File("E:/Global Model/Pearson" + outdate[datenum] +".csv");
        File Deviation = new File("E:/Global Model/AbsDeviation" + outdate[datenum] +".csv");
        File PercentageDeviation = new File("E:/Global Model/PercentageDeviation" + outdate[datenum] +".csv");
        PrintWriter pearsonp = new PrintWriter(pearson);
        PrintWriter devp = new PrintWriter(Deviation);
        PrintWriter percentp = new PrintWriter(PercentageDeviation);
        for (int i = 0; i < country.size(); i++) {
            pearsonp.print(country.get(i)+",");
            devp.print(country.get(i)+",");
            percentp.print(country.get(i)+",");
        }
        pearsonp.println();
        devp.println();
        percentp.println();
        for (int i = 0; i < country.size(); i++) {
            pearsonp.print(PearsonProduct.get(i)+",");
            devp.print(Dev.get(i)+",");
            percentp.print(PercentageDev.get(i)+",");
        }
        pearsonp.close();
        devp.close();
        percentp.close();
